                ...
                <form action="" method="POST">
                    ...
                    if (isset($_POST["timkiem"])) {
                        ...
                        if ($s == "") {
                            ...
                            echo '<br>';
                            echo '<a style="color:black" href="quanlydh.php">Tất cả</a>';
                            echo "";
                        } else {
                            $conn = mysqli_connect("localhost", "root", "", "web1");
                            $sql = "SELECT * FROM cc WHERE username LIKE '%$s%'";
                            $kq = mysqli_query($conn, $sql);
                            $count = mysqli_num_rows($kq);
                            if ($count < 0) {
                                // echo "khong tim thấy kết quả với từ khóa ".$s;
                                echo 'Không tìm thấy kết quả phù hợp';
                                echo '<a href="quanlydh.php">Tất cả</a>';
                            } else {
                    ...
                    ?>
                                <table class="table table-striped" style="width: 80%;">
                                    ...
                                    while ($row =  mysqli_fetch_array($kq)) {
                                        ...
                                        echo '<td>' . $row['ten'] . '</td>';
                                        echo '<td style="padding-left:40px">' . $row['soluong'] . '</td>';
                                        echo '<td>' . $row['gia'] . '</td>';
                                        echo '<td><a style="color:black" href="duyetdon.php?ID=' . $row['ID'] . '">' . $row['duyet'] . '</a></td>';
                                        echo '</tr>';
                                        $stt++;
                                    }
                                    ?> <?php
                            ...
                            <?php
                                ...
                                while ($row1 =  mysqli_fetch_array($kq1)) {
                                    ...
                                    echo '<td>' . $row1['ten'] . '</td>';
                                    echo '<td style="padding-left:40px">' . $row1['soluong'] . '</td>';
                                    echo '<td>' . $row1['gia'] . '</td>';
                                    echo '<td><a style="color:black" href="duyetdon.php?ID=' . $row1['ID'] . '">' . $row1['duyet'] . '</a></td>';
                                    echo '</tr>';
                                    $stt++;
                                }
                            }
